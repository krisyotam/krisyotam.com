...
interface EssaysTagsTableProps {
  tags: EssayTag[];
  searchQuery: string;
}

export function EssaysTagsTable({ tags, searchQuery }: EssaysTagsTableProps) {
  const [filteredTags, setFilteredTags] = useState<EssayTag[]>(tags);
  ...
  function getTagUrl(tag: EssayTag) {
    return `/essays/tag/${tag.slug}`;
  }
