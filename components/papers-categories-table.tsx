...
interface PapersCategoriesTableProps {
  categories: PaperCategory[];
  searchQuery: string;
}

export function PapersCategoriesTable({ categories, searchQuery }: PapersCategoriesTableProps) {
  const [filteredCategories, setFilteredCategories] = useState<PaperCategory[]>(categories);
  ...
  function getCategoryUrl(category: PaperCategory) {
    return `/papers/${category.slug}`;
  }
