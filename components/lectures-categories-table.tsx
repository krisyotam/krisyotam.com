...
interface LecturesCategoriesTableProps {
  categories: LectureCategory[];
  searchQuery: string;
}

export function LecturesCategoriesTable({ categories, searchQuery }: LecturesCategoriesTableProps) {
  const [filteredCategories, setFilteredCategories] = useState<LectureCategory[]>(categories);
  ...
  function getCategoryUrl(category: LectureCategory) {
    return `/lectures/${category.slug}`;
  }
