...
interface LecturesTagsTableProps {
  tags: LectureTag[];
  searchQuery: string;
}

export function LecturesTagsTable({ tags, searchQuery }: LecturesTagsTableProps) {
  const [filteredTags, setFilteredTags] = useState<LectureTag[]>(tags);
  ...
  function getTagUrl(tag: LectureTag) {
    return `/lectures/tag/${tag.slug}`;
  }
