...
interface PapersTagsTableProps {
  tags: PaperTag[];
  searchQuery: string;
}

export function PapersTagsTable({ tags, searchQuery }: PapersTagsTableProps) {
  const [filteredTags, setFilteredTags] = useState<PaperTag[]>(tags);
  ...
  function getTagUrl(tag: PaperTag) {
    return `/papers/tag/${tag.slug}`;
  }
