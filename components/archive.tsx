 ...
 * Description: File browser component for displaying object storage buckets.
 *              Supports tree navigation, search, and list/grid view modes.
 * ============================================================================
 */
...
/**
 * Build a tree of nodes per bucket from the storage data
 */
function buildBucketsTree(data: StorageData): Node[] {
  const buckets: Node[] = [];
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (!map.has(subpath)) {
          const node: Node = {
            ...
            type: isDirectory
              ? "directory"
              : (obj?.key.split(".").pop() ?? "file").toLowerCase(),
...
const segmentToBucket: Record<string, string> = Object.fromEntries(
  Object.entries(bucketToSegment).map(([k, v]) => [v, k])
);
...
export default function ArchivesComponent({
  ...
  const buckets = useMemo(() => buildBucketsTree(data), [data]);
  ...
  const itemsToShow = useMemo(() => {
    ...
    const results: Node[] = [];
    const walk = (n: Node) => {
      if (n.name.toLowerCase().includes(q) && n !== currentNode)
        results.push(n);
  ...
  const openNode = (node: Node) => {
    if (isFile(node) && node.meta?.public_url) {
      ...
      if (key) {
        const root =
          rootMap[bucket] ?? rootMap[currentBucket ?? "doc"] ?? `${baseOrigin}/doc`;
  ...
  const getDisplayUrl = () => {
    const origin =
      typeof window !== "undefined"
        ? window.location.origin
        : "https://krisyotam.com";
    const seg = currentBucket
      ? (bucketToSegment[currentBucket] ?? currentBucket)
      : "";
      ...
      {!currentBucket ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {buckets.map((b) => (
            <Card
              key={b.name}
              className="rounded-none border-border cursor-pointer hover:bg-secondary/50 transition-colors"
              onClick={() => enterFolder(b)}
            >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {itemsToShow.map((item) => (
            <Card
              key={item.id}
              className="rounded-none border-border hover:bg-secondary/50 transition-colors"
            >
              <CardContent
              ...
              <CardFooter className="p-4 pt-0 text-xs text-muted-foreground">
                {isFile(item) ? (
                  <div className="flex justify-between items-center text-xs text-muted-foreground">
                    ...
                    <span>
                      {item.meta?.last_modified
                        ? new Date(item.meta.last_modified).toLocaleString(
                            "en-US",
                            {
