...
3. **Globally Unique** — No two pieces of content across ANY route can share the same slug
   - This is required for sexy URLs (`krisyotam.com/{slug}`) and magic URLs to function
   - Before creating, verify the slug doesn't exist in: papers, blog, essays, notes, diary, progymnasmata, reviews, verse, fiction, news, ocs
   - If a conflict exists, make the slug more specific (e.g., `stoicism` → `stoicism-daily-practice`)
   - **Review collision rules** (reviews are the only route where media-type collisions occur):
     - First created gets the bare slug. Subsequent collisions disambiguate with creator or year.
     - Disambiguation hierarchy (pick the first that resolves it):
       1. `-director` or `-mangaka` or `-author` (e.g., `dune-villeneuve`, `koukaku-kidoutai-shirow`)
       2. `-year` (e.g., `dune-2021`, `little-women-2019`)
       3. `-creator-year` if both are needed (e.g., `hamlet-branagh-1996`)
     - NEVER use media-type suffixes like `-manga`, `-film`, `-book` — these are ugly and redundant
     - Examples:
       - `pride-and-prejudice` (book, Jane Austen) + `pride-and-prejudice-wright` (2005 film, Joe Wright)
       - `koukaku-kidoutai` (1995 anime, Oshii) + `koukaku-kidoutai-shirow` (manga, Masamune Shirow)
       - `dune` (1984 Lynch) + `dune-villeneuve` (2021 Villeneuve)
     - Use the Japanese title for anime/manga when distinct; use the English title for Western media
...
Content is **divorced from infrastructure**. Each content type lives in its own git repo under `~/content/`:

```
~/content/
  blog/          essays/        fiction/       diary/
  news/          notes/         ocs/           papers/
  progymnasmata/ reviews/       verse/
```

All MDX files are **flat** — no category subdirectories. The database (`content.db`) tracks categories.

These repos are mounted into the site via:
- **Local dev**: Symlinks (`src/app/(content)/TYPE/content -> ~/content/TYPE`)
- **Docker/CI**: Git submodules (`.gitmodules`) or direct clone in Dockerfile

## Sexy URLs
Every content slug is accessible at `krisyotam.com/{slug}` via Next.js rewrites.
Generated by `scripts/sexy-urls.js` (runs as `prebuild` step), output: `public/data/sexy-urls.json`.
Canonical routes (`/{type}/{category}/{slug}`) still work. The `[slug]/page.tsx` catch-all
...
└── content/                     # Symlink to ~/content/[route]/ (or git submodule)
    └── *.mdx                    # Flat MDX files — no category subdirectories
```
...
export default async function DetailPage({ params }) {
  ...
  const Content = (await import(`@/app/(content)/route/content/${slug}.mdx`)).default;
