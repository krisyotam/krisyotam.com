 ...
 * JSON data files (pages, posts, notes, tags, series, categories) and presents the most 
...
if (typeof window.URL_SUGGESTER_LOADED === 'undefined') {
  ...
  const dataUrls = {
    ...
    series: '/api/data?file=series.json',
  ...
  async function collectPaths() {
    ...
    try {
      ...
      // Series
      const seriesData = await fetchData(dataUrls.series);
      if (seriesData && seriesData.series) {
        seriesData.series.forEach(series => {
          if (series.slug) {
            allPaths.push(`/series/${series.slug}`);
          }
        });
      }
