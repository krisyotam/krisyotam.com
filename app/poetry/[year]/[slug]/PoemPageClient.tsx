...
export default function PoemPageClient({ params }: { params: { year: string; slug: string } }) {
  ...
  const poem = poemsData.find((p: any) => p.year === year && p.slug === slug) as Poem | undefined

  if (!poem) {
    notFound()
  }

  // Extract stanzas from the poem object
  const stanzas: string[] = []
  let stanzaIndex = 1

  while (poem[`stanza${stanzaIndex}`]) {
    stanzas.push(poem[`stanza${stanzaIndex}`])
    stanzaIndex++
  }
            ...
            <div className="poetry-content prose dark:prose-invert max-w-none">
              {stanzas.length > 0 ? (
                stanzas.map((stanza, index) => (
                  <div key={index} className="poetry-stanza">
                    {stanza}
                  </div>
                ))
              ) : (
