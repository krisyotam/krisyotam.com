...
export async function generateStaticParams() {
  const lectureNotes = lectureNotesData as unknown as LectureNote[];
  return lectureNotes.map((item) => ({
    category: slugifyCategory(item.category ?? ""),
    year: new Date(item.dateStarted ?? "1970-01-01").getFullYear().toString(),
...
export async function generateMetadata(
  ...
  const item = lectureNotes.find((r) => {
    ...
    const itemYear = new Date(r.dateStarted ?? "1970-01-01").getFullYear();
...
export default function LectureNotePage({
  ...
  return <LectureNoteDetail lectureNote={{
    ...item,
    id: item?.id ?? "",
    title: item?.title ?? "",
