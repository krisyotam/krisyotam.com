 ...
 * Date: 2026-01-04
 * Filename: page.tsx
 * Description: Source archives page displaying mirrored repositories stored
 *              in the src bucket. Protected by password authentication.
 * ============================================================================
 */
...
export default function SrcPage() {
  ...
  const fetchStorageData = async () => {
    setIsLoading(true);
    try {
      const response = await fetch("/api/storage?bucket=src");
      if (!response.ok) {
    ...
    <div className="relative min-h-screen bg-background text-foreground">
      <div className="container max-w-[672px] mx-auto px-4 pt-16 pb-8">
        <PageHeader
          ...
          confidence="certain"
          importance={7}
          preview="an extensive list of src repositories I routinely updated from the remote since 2025"
        />

        {storageData && (
          <ArchivesComponent defaultBucket="src" data={storageData} />
        )}
