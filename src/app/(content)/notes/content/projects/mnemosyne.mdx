...
# @author Kris Yotam
# @type notes
# @path src/app/(content)/notes/content/projects/mnemosyne.mdx
# ==============================================================================
...
importance: 9
author: "Kris Yotam"
description: "a overview of my lifelogging system since 2025"
tags: [Personal, Lifelogging, System]
category: projects
sequences: []
cover: https://krisyotam.com/doc/assets/covers/notes/mnemosyne.png
# ==============================================================================
...
Testing the new code component:

<CodeBlock filename="metrics_logger.py" language="Python" caption="Example Python code for data logging">
{`import datetime
import json
...
logger.log("sleep_hours", 7.5)
logger.log("mood", "good")
`}
</CodeBlock>

<CodeBlock filename="App.scala" language="Scala">
{`object MetricsApp extends App {
  case class Metric(name: String, value: Double, timestamp: Long)

  val metrics = List(
    Metric("sleep_hours", 7.5, System.currentTimeMillis()),
    Metric("mood_score", 8.0, System.currentTimeMillis())
  )

  metrics.foreach(m => println(s"\${m.name}: \${m.value}"))
}
`}
</CodeBlock>
...
logMetric :: String -> Double -> Metric
logMetric = Metric

main :: IO ()
main = do
  let sleep = logMetric "sleep_hours" 7.5
  print sleep
`}
</CodeBlock>
...
**Artboards** have become the de-facto standard to convey how a certain UI is supposed to be rendered for
...
Once rendered, I incorporated a `[+]` icon (which you can click above) to modify the parameters with which the view was invoked and rendered.
...
<FootNotes>
  ...
  <FootNote id="2">Artboards were initially introduced by Illustrator as a way of grouping related designs together, such as a company's business stationary. Sketch popularized its usage to represent designs targeting varying devices and screen sizes, and Photoshop is now incorporating the feature.</FootNote>
</FootNotes>
