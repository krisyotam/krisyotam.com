...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log(`Fetching recently watched shows from Trakt (limit: ${limit})`);
    ...
    console.log(`Transformed ${transformedShows.length} recently watched shows`);
    return NextResponse.json(transformedShows);
    
  } catch (error) {
    console.error('Error fetching Trakt recently watched shows:', error);
