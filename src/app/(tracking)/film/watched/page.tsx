...
export default function WatchedPage() {
  ...
  useEffect(() => {
    async function fetchWatched() {
      try {
        const res = await fetch("/api/film?resource=watched&limit=0")
  ...
  const allDecades = Array.from(new Set(watched.map(f => `${Math.floor(f.year / 10) * 10}s`))).sort()
  ...
  return (
    <>
      <PageHeader
        title="Watched Films"
        preview="Complete list of all films I've watched"
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-4 flex-wrap">
            ...
            <div className="px-3 py-1.5 bg-muted/50 dark:bg-[hsl(var(--popover))] border border-border text-sm whitespace-nowrap">
              ...
              <span className="ml-1 text-muted-foreground">{filtered.length === 1 ? 'film' : 'films'}</span>
            </div>
          </div>

          {/* Film Grid */}
          {filtered.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-8">No films match your filters.</p>
          ) : (
                ...
                >
                  {film.posterUrl ? (
                    ...
                    <div className="w-full h-full flex items-center justify-center bg-muted p-4">
                      <span className="text-muted-foreground text-xs text-center line-clamp-3">{film.title}</span>
                    </div>
                  )}
                  <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity">
                    <div className="absolute bottom-0 left-0 right-0 p-3">
                      <p className="text-white text-sm font-medium line-clamp-2">{film.title}</p>
