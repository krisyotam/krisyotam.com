...
const getEasternZodiac = (year: number) => {
  ...
  const elementIndex = (year - 1900) % 10
  const element = elements[elementIndex]
  const fullName = `${element} ${animals[animalIndex]}`

  // The 60-year sexagenary cycle: 12 animals x 5 elements
  // Each specific element-animal pairing recurs every 60 years
  const prevCycleYear = year - 60
  const nextCycleYear = year + 60

  return {
    animal: animals[animalIndex],
    element,
    fullName,
    cycle: `${prevCycleYear}, ${year}, ${nextCycleYear}`,
    description: "The Eastern zodiac combines a 12-year animal cycle with a 5-element cycle, creating a 60-year sexagenary cycle. Each specific pairing recurs every 60 years.",
  }
}
      ...
      <Tabs defaultValue="age" className="mt-2">
        ...
        <TabsContent value="age">
          <div className="pt-4 space-y-4">
            <div className="flex items-baseline justify-between text-sm text-muted-foreground px-1">
              <span>Born {format(ageInfo.birthDate, "MMMM d, yyyy")} at {format(ageInfo.birthDate, "h:mm a")}</span>
              <span className="font-mono text-xs">{ageInfo.ageInTotalSeconds.toLocaleString()}s</span>
            </div>
          ...
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-2 gap-3">
            <div className="border border-border p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Western</span>
                <span className="text-xs text-muted-foreground">{ageInfo.westernZodiac.dates}</span>
              </div>
              <div className="text-2xl font-bold">{ageInfo.westernZodiac.sign}</div>
              <p className="text-sm text-muted-foreground leading-relaxed">{ageInfo.westernZodiac.description}</p>
            </div>
            <div className="border border-border p-4 space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Eastern</span>
                <span className="text-xs text-muted-foreground">60-year cycle</span>
              </div>
              <div>
                <div className="text-2xl font-bold">{ageInfo.easternZodiac.fullName}</div>
                <span className="text-xs text-muted-foreground">{ageInfo.easternZodiac.cycle}</span>
              </div>
              <p className="text-sm text-muted-foreground leading-relaxed">{ageInfo.easternZodiac.description}</p>
            </div>
          </div>
        </TabsContent>
        ...
        <TabsContent value="numerology">
          <div className="pt-4 grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div className="border border-border p-4 space-y-3">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Life Path</span>
              <div className="text-4xl font-bold tabular-nums">{ageInfo.lifePathNumber.number}</div>
              <p className="text-xs text-muted-foreground leading-relaxed">{ageInfo.lifePathNumber.description}</p>
            </div>
            <div className="border border-border p-4 space-y-3">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Soul Urge</span>
              <div className="text-4xl font-bold tabular-nums">{ageInfo.soulUrgeNumber.number}</div>
              <p className="text-xs text-muted-foreground leading-relaxed">{ageInfo.soulUrgeNumber.description}</p>
            </div>
            <div className="border border-border p-4 space-y-3">
              <span className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Destiny</span>
              <div className="text-4xl font-bold tabular-nums">{ageInfo.destinyNumber.number}</div>
              <p className="text-xs text-muted-foreground leading-relaxed">{ageInfo.destinyNumber.description}</p>
            </div>
          </div>
        </TabsContent>
      </Tabs>
